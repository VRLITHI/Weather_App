...
private void displayvideo(){
        String path = UpdateUI.getVideoId(condition, update_time, sunrise, sunset);

        if(path == null){
           path =  "android.resource://com.aniketjain.weatherapp/"+R.raw.clear_day;
        }
    Uri u = Uri.parse(path);
        binding.bgvideo.setVideoURI(u);
    ...
    protected void onResume() {
        super.onResume();
        checkConnection();
        binding.bgvideo.resume();
    }

    @Override
    protected void onPause() {
        super.onPause();
        binding.bgvideo.suspend();
    }
